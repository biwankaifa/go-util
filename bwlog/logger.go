...
func init() {
	...
	ProjectLogFile := "./logs/test-access.log"

	loggers, errLog = NewJSONLogger(
		WithDisableConsole(),
		WithField("domain", fmt.Sprintf("%s[%s]", "test", env.Active().Value())),
		WithTimeLayout("2006-01-02 15:04:05"),
		WithFileP(ProjectLogFile),
	)
...
// m 对外获取数据库连接对象db
func (m *Logs) m() *zap.Logger {
	return loggers
}

// Error 对外获取数据库连接对象db
func (m *Logs) Error() (error error) {
	return errLog
}
